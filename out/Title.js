...
export class Title {
    ...
    title = new SimpleBox(transparent);
    counters = [];
    constructor() {
    ...
    render(dt) {
        ...
        for (let i = 0; i < this.counters.length; i++) {
            const t = this.counters[i];
            t.box.render(dt);
            myctx.drawText(t.box.smoothPos.add(t.box.smoothSize.mul(0.5)), t.t, { size: textSize2, align: "center", baseline: "middle" }, { color: getColor(0.3) });
        }
        t.render(dt);
