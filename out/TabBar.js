...
export class TabBar {
    ...
    pointer = new SimpleBox(getColor(0.75));
    buttons = [];
    t = 0;
    constructor() {
    ...
    render(dt) {
        ...
        for (let i = 0; i < this.buttons.length; i++) {
            ...
            myctx.drawText(this.buttons[i].box.smoothPos.add(this.buttons[i].box.smoothSize.mul(0.5)), this.buttons[i].name, { size: textSize, align: "center", baseline: "middle" }, { color: getColor(0.25) });
        }
    }
}
